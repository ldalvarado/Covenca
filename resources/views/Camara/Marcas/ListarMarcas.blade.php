...
@section('contenido')
    ...
    <div class="br-pagebody mg-t-5 pd-30">
      ...
      @include('Camara.Marcas.CrearMarca')
      @include('Camara.Marcas.EditarMarca')
      <script>
           $(document).ready(function(){
            ActivateNavLink("nav-link_ama");
            });
                var table = $('#tableMarca').DataTable({
                ajax: {
                url: '<?php echo route('CamaraListadoMarca') ?>',
                type: "POST",                
                data: {
    ...
    function eliminar_marca(id){
            ...
            var settings = {
                "url": '<?php echo route('CamaraDeleteMarca') ?>',
                "method": "DELETE",
