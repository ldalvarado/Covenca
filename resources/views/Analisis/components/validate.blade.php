                               ...
                               <div class="form-layout form-layout-1 p-1">
                                       <div class="d-flex-column align-items-center justify-content-around ht-auto bd pd-x-20 pd-y-10"><!--Vehículo-->
                                          ...
                                          <dic class="row pd-y-20 pd-md-y-0">
                                              <div class="col-12">
                                                <h4 class="tx-info">Información del vehiculo</h4>
                                              </div>
                    ...
                    <div class="d-flex-column align-items-center justify-content-around mg-t-15 bg-gray-100 ht-auto bd pd-x-20 pd-y-10"><!--Producto-->
                      ...
                      <div class="col-md-4"><!--Marca-->
                              <div class="form-group mg-b-10-force">
                                  ...
                                  <select class="form-control" name="valvula" data-placeholder="Seleccione la Valvula" required>
                                      @foreach($valvula as $marcaCau)
                                          <option value="{{$marcaCau['id']}}" {{$info_product->idMarcaCaucho==$marcaCau['id'] ? 'selected' : ''}}>{{$marcaCau['descripcion']}}</option>
                                      @endforeach
                                  </select>
                              </div>
                    ...
                    -->
                        ...
                        <div class="form-group mg-b-10-force">
                            ...
                            <div class="d-flex row justify-content-around">
                                <label class="d-flex rdiobox rdiobox-primary">
                                    <input onchange="GetDisenoByFrio()" type="radio" name="tipo_renovado" id="cold" value="cold" @if($info_product->tipoRenovado) checked @endif ><span>Frío</span>
                                </label>

                                <label class="d-flex rdiobox rdiobox-primary">
                                    <input onchange="GetDisenoByFrio()" type="radio" name="tipo_renovado" id="warm" value="warm" @if(!$info_product->tipoRenovado) @endif><span>Caliente</span>
                                </label>
                            </div>
                        </div>
                    </div><!-- col-4 -->
                      ...
                      <div class="col-md-4" id="input_boleta"><!--Boleta-->
                        <div class="form-group mg-b-10-force">
                            ...
                            <input disabled class="form-control" type="text" name="boleta" required value="{{$info_product->boleta}}" id="boletaNeumatico">
                        </div>
                    </div><!-- col-4 -->
                      @endif
...
<script>
    ...
    function GetDisenoByFrio(){
        let cold = 1;
        if(document.getElementById('warm') != null ){
            console.log("aveces");
        ...
        let marcaproducto = document.getElementById('diseno');
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for(let i = 0 ; i < elementos.length ; i++){
                    ...
                    marcaproducto.value = {{$info_product->idDisenoBanda}};
            }
        })
    }
    function condiciones() {
        ...
        var condiciones_reales = [];
        var cond = [];
        var condActv = [];
        let count_temp = 0;
        for(let i = 0 ; i < condiciones.length; i++){
            let skip = false;
            for(let real = 0 ; real < condiciones_reales.length; real++){
                if(condiciones[i].idCondicion == condiciones_reales[real].idCondicion){
                    skip = true;
                    break;
                }
            ...
            if(!skip){
                condiciones_reales[count_temp] = condiciones[i];
                count_temp++;
            }
        }
        condiciones = condiciones_reales;
        console.log(condiciones);
        count = 0;
        var innerhtml = ""
